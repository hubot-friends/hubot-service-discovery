...
export class EventStore extends EventEmitter {
  constructor(options = {}) {
    ...
    this.persistEventTypes = Array.isArray(options.persistEventTypes) ? options.persistEventTypes : null
  ...
  async appendEvent(event) {
    ...
    if (!this.shouldPersistEvent(eventWithTimestamp)) {
  ...
  shouldPersistEvent(event) {
    if (!this.persistEventTypes) {
      return true
    }

    return this.persistEventTypes.includes(event.type)
  }
}
